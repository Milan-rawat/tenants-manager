...
class TenantEdit extends Component {
  ...
  updateTenant = async () => {
    ...
    for (let i = 0; i < allTenants.length; i++) {
      if (i !== this.indexNumber) newTenants.push(allTenants[i]);
      if (i === this.indexNumber) newTenants.push(this.state.tenant);
    }
        ...
        <div className="tenantsForm">
          ...
          <div className="addButton">
            <div className="buttons">
              <Link
                to={{
                  pathname: "/tenantDetail" + this.indexNumber,
                }}
              >
                <button>Cancel</button>
              </Link>
              <button onClick={this.updateTenant}>Update</button>
            </div>
          </div>
        </div>
      </div>
